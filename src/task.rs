...
pub enum RateLimitDecision {
    ...
    Allowed,
    /// The global rate limit has been reached according to the custom
    /// algorithm.
    ///
    /// The task execution will be aborted and its `run_at` timestamp will be
    /// shifted into the future by the specified `Duration`.
    Limited {
        /// The precise interval this task should be put to sleep before
        /// returning to the Pending queue.
...
/// A "Bring Your Own" Rate Limiting system that allows developers to
/// defensively throttle their queues using custom algorithms (e.g. GCRA, Token
/// Bucket, Fixed Window) often backed by external systems like Redis.
///
/// If a task is rate limited, the worker will gracefully pause the task and
/// shift its scheduling into the future, preventing infinite "Busy Waiting"
/// database loops.
