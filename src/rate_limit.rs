...
//!
//! The concept of Rate Limiting in Underway was split into two highly optimized paradigms: **Local Concurrency Control** and **Global API Throttling**.
//!
//! ## 1. Local Pool Control (`max_concurrency`)
//!
//! If you just need to ensure your worker pool doesn't crash your server by processing 1,000 tasks at the exact same instant, use `max_concurrency`.
//!
//! This is a fast, database-native lock that simply ensures no more than `N` tasks are in the `InProgress` state on the table at any given time.
//!
//! ```rust,no_run
...
//!
//! Often, tasks hit an external SaaS API (like Stripe or Spotify) that enforces strict rate limits like **"Max 100 requests per minute"**. Using `max_concurrency` here doesn't work, because you could technically process 100 tasks within 2 seconds (concurrency 5) and still trigger a 429 Too Many Requests ban from the API.
//!
//! Instead, you need a true global rate limit logic that behaves as a Token Bucket. Because you likely run multiple horizontal worker servers, the state of this limit *must* be shared.
//!
//! Underway now offers a completely native, out-of-the-box solution using PostgreSQL itself as the global state mechanism:
//!
//! ```rust,no_run
//! # use sqlx::{Postgres, Transaction};
//! # use underway::{Task, task::Result as TaskResult, Queue};
...
//! ```
//!
//! By simply supplying those numbers, Underway generates a `RateLimiter` plugin that tracks tokens in the `underway.global_rate_limit` table. Every single container you boot up will flawlessly coordinate together so that the summation of their throughput never exceeds 100 hits per 60 seconds.
//!
//! ### Why this saves massive CPU/Database load (No "Busy-Waiting"):
//!
//! When a worker fetches a task and realizes the global rate limit bucket is currently empty, it receives exactly how many milliseconds are left until the next token is mathematically available.
//!
//! Instead of executing the task and violating the downstream limit, or crashing the task and causing noisy Retry loops, the worker intercepts the task and dynamically postpones it in memory:
//!
//! The worker simply sleeps the specific Task Future for `X` milliseconds locally, and then seamlessly proceeds to execute it once the exact bucket timestamp clears, without yielding the task back to PostgreSQL!
//!
//! This means the web workers and the PostgreSQL system undergo **0 compute cost** waiting for the bucket to refill, and tasks are executed natively at the exact maximum allowable throughput of the API!
//!
//! ### Managing the Bucket State
//! Underway deliberately avoids exposing raw database internals to consumers. If you need to manually intervene (e.g., during tests or because a bucket became stuck), you should use the provided helper rather than writing raw `DELETE` SQL queries:
//!
//! ```rust,no_run
//! # use sqlx::{Postgres, Transaction};
//! # use underway::{Task, task::Result as TaskResult, Queue};
...
//! In addition to Fixed Windows, the true native `TokenBucket` algorithm is perfectly supported! If you want "smooth" execution (e.g. consistently dripping 2 tasks per second) but want to allow a short burst of up to 10 tokens:
//!
//! ```rust,no_run
//! # use sqlx::{Postgres, Transaction};
//! # use underway::{Task, task::Result as TaskResult, Queue};
...
//! If you want the smooth execution of a Token Bucket but want maximum database performance via a 1-column tracking state (`Theoretical Arrival Time`), use `GCRA`:
//!
//! ```rust,no_run
//! # use sqlx::{Postgres, Transaction};
//! # use underway::{Task, task::Result as TaskResult, Queue};
...
/// The specific algorithm and configuration used by the distributed rate limiter.
#[derive(Debug, Clone, PartialEq)]
#[non_exhaustive]
pub enum RateLimitAlgorithm {
    /// A Fixed Window algorithm that resets exactly `window` duration after the first request.
    ///
    /// This perfectly caps total throughput within the defined window, making it ideal
    /// for adhering to remote HTTP APIs with "100 requests per minute" type constraints.
    FixedWindow {
    ...
    /// A token bucket algorithm where tasks consume tokens that refill at a constant rate over time.
    ///
    /// This provides perfectly smooth execution (e.g., exactly 2 tasks per second) while allowing
    /// for short concurrency bursts up to the `max_capacity`.
    TokenBucket {
        /// The maximum burst capacity of the bucket (the maximum tokens that can be accumulated).
        max_capacity: f64,
        /// How rapidly the bucket refills (tokens per second).
        refill_rate_per_sec: f64,
    },
    /// Generic Cell Rate Algorithm (GCRA).
    /// Mathematically equivalent to a Token Bucket but tracks state natively in PostgreSQL
    /// using only a single Theoretical Arrival Time (TAT) timestamp for ultimate efficiency.
    GCRA {
        /// The cost of a single task (e.g., 1 task per 500ms).
        emission_interval: Duration,
        /// The maximum burst tolerance. A value of 0 means strict rate spacing.
        /// A value of `emission_interval * 10` would allow an immediate burst of 10 tasks.
        delay_variation_tolerance: Duration,
    },
}

/// A native rate limiter that uses PostgreSQL to track and enforce a global rate limit.
/// Because it uses a centralized database, limits are strictly enforced across all
/// independent distributed workers.
#[derive(Debug, Clone)]
        ...
        Box::pin(async {
            match &self.algorithm {
                RateLimitAlgorithm::FixedWindow { max_tokens, window } => {
                    ...
                    // Fetch the current state of the bucket, resetting it if the time window has expired
                    let row = sqlx::query!(
                        r#"
                ...
                RateLimitAlgorithm::TokenBucket {
                    ...
                    // We calculate tokens newly generated since `window_start` and subtract them from `tokens_used`, clamped horizontally by 0 and max_capacity.
                    let row = sqlx::query!(
                        r#"
                ...
                RateLimitAlgorithm::GCRA {
                    ...
                    // If allowed, we update the DB's TAT. If limited, we do NOT update the DB, and return wait time.
                    let row = sqlx::query!(
                        r#"
                        WITH current_state AS (
...
///
/// This safely removes the tracking state for the given `id` in the `underway.global_rate_limit` table.
/// It is primarily designed for use in system testing or to manually clear a stuck bucket.
